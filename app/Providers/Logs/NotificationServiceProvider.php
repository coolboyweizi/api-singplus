...
namespace SingPlus\Providers\Logs;
...
class NotificationServiceProvider extends ServiceProvider
{
    ...
    {
        $this->app->singleton('log.notification', function () {
    ...
    {
        $log->useFiles(
            $this->app->storagePath().'/logs/notification.log',
            $this->logLevel()
        );

        foreach ($log->getMonolog()->getHandlers() as $handler) {
          if ($handler instanceof \Monolog\Handler\StreamHandler) {
            $lineFormat = "[%datetime%]___%channel%.%level_name%:___%message%___%context%___%extra%\n";
            $lineFormatter = new LineFormatter($lineFormat, null, true, true);
    ...
    {
        $log->useDailyFiles(
            $this->app->storagePath().'/logs/notification.log', $this->maxFiles(),
            $this->logLevel()
        );
